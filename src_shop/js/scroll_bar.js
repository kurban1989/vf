;(function (factory) {
    ...
    function SimpleBar (element, options, box, autoHideOpt) {
        this.el = element,
        this.$el = $(element),
        this.$track,
        this.$scrollbar,
        this.dragOffset,
        this.flashTimeout,
        this.simplebar        = box === undefined || box === '' ? 'simplebar' : box,
        ...
        this.offsetAttr       = 'top';
        this.autohide         = autoHideOpt === undefined ? true : false;

        this.options = $.extend({}, SimpleBar.DEFAULTS, options);

        Object.defineProperty(this.options, "addSimple", { set: function (x) { this.simplebar = x; } });

        this.options.addSimple = this.simplebar;
